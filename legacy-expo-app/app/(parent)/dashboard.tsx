...
export default function DashboardScreen() {
  ...
  const { setupChecklistDismissed, setSetupChecklistDismissed } = useSettingsStore();
  const {
    family,
    children,
    members,
  ...
  const loadAll = useCallback(async () => {
    ...
    await useFamilyStore.getState().fetchAllFamilyBuckets();
  }, []);

  // Depend on profile.family_id so browser refreshes still trigger a full load
  // once the restored profile is available.
  useEffect(() => {
    let cancelled = false;
  ...
  const getChildBalance = (childId: string) => {
    if (childBalances[childId] != null) {
      return childBalances[childId];
    }
  ...
  const setupSteps = [
    {
      ...
      route: "/(parent)/add-children",
    },
    {
      label: "Set up savings buckets",
      done: bucketTemplates.length > 0,
      route: "/(parent)/bucket-templates",
    },
    {
      label: "Create your first chore",
      done: chores.length > 0,
      route: "/(parent)/chores",
    },
  ];
  const completedSetupSteps = setupSteps.filter((step) => step.done).length;
  const showSetupChecklist =
    !setupChecklistDismissed && completedSetupSteps < setupSteps.length;

  useEffect(() => {
    if (completedSetupSteps === setupSteps.length && !setupChecklistDismissed) {
      setSetupChecklistDismissed(true);
    }
  }, [completedSetupSteps, setupChecklistDismissed, setSetupChecklistDismissed, setupSteps.length]);
  ...
  return (
    ...
    >
      ...
      {showSetupChecklist && (
        <Card mode="outlined" style={[styles.setupCard, { borderColor: theme.colors.primary }]}>
          <Card.Content>
            <View style={styles.setupHeader}>
              <View style={styles.setupTitleRow}>
                <MaterialCommunityIcons name="rocket-launch" size={20} color={theme.colors.primary} />
                <Text variant="titleSmall" style={[styles.setupTitle, { color: theme.colors.primary }]}>
              ...
              <IconButton
                icon="close"
                size={18}
                onPress={() => setSetupChecklistDismissed(true)}
                style={styles.setupClose}
              />
            </View>
            <ProgressBar
              progress={completedSetupSteps / setupSteps.length}
            ...
            <View style={styles.setupSteps}>
              {setupSteps.map((step) => (
                <Button
                  ...
                  disabled={step.done}
                  contentStyle={styles.setupStepButton}
                  labelStyle={[
                    styles.setupStepLabel,
                    step.done && { textDecorationLine: "line-through", color: theme.colors.onSurfaceVariant },
                  ]}
                >
                  {step.label}
                </Button>
                ...
                >
                  <Card.Content style={styles.childCardContent}>
                    ...
                    <IconButton
                      ...
                      size={20}
                      onPress={() =>
                        router.replace(`/(child)/child-dashboard?id=${item.id}`)
                      }
                    />
      ...
      <View style={styles.section}>
        ...
        <View style={styles.actionsGrid}>
          ...
          <Card
            style={styles.actionCard}
            mode="elevated"
            onPress={() => router.push("/(parent)/family-tasks-dashboard")}
          >
            <Card.Content style={styles.actionContent}>
              <MaterialCommunityIcons
              ...
              <Text
                variant="labelLarge"
                style={{ marginTop: spacing.xs, fontWeight: "700", textAlign: "center" }}
              >
                Task Board
              </Text>
            </Card.Content>
