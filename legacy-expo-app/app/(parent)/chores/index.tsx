...
export default function ChoresScreen() {
  ...
  const handleSyncGoogleTasks = async () => {
    ...
    if (result.errors.length > 0) {
      showError(result.errors[0].includes('google_auth_expired') ? 'Google access expired. Reconnect Google Tasks in Settings.' : result.errors[0]);
    } else {
      showSuccess(`Synced ${result.synced} task${result.synced === 1 ? '' : 's'} from Google.`);
    }
  };
...
const styles = StyleSheet.create({
  ...
  syncButton: { marginBottom: spacing.md, alignSelf: 'flex-start' },
