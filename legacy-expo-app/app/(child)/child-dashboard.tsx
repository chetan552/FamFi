...
let pendingChildRedirectPath: string | null = null;
...
export default function ChildDashboardScreen() {
  ...
  useEffect(() => {
    pendingChildRedirectPath = null;
  }, [pathname]);

  const replaceIfNeeded = useCallback((
    ...
    if (currentPathname === targetPathname || pendingChildRedirectPath === targetPathname) {
      return;
    }

    pendingChildRedirectPath = targetPathname;
    router.replace(href);
  }, [pathname]);
  ...
  useEffect(() => {
    if (initialized && !id) {
      ...
      if (currentProfile?.role === 'child') {
        replaceIfNeeded({ pathname: '/(child)/child-dashboard', params: { id: currentProfile.id } }, '/child-dashboard');
      } else {
        replaceIfNeeded('/(parent)/dashboard', '/dashboard');
      }
    }
  }, [initialized, id, replaceIfNeeded]);
  ...
  }, [initialized, session, replaceIfNeeded]);
