...
export default function WithdrawScreen() {
  ...
  const getBucketBalance = (childIdParam: string, templateIdParam: string): number => {
    const bucket = allFamilyBuckets.find(
      b => b.child_id === childIdParam && b.template_id === templateIdParam
    );
    return bucket?.cached_balance ?? 0;
  };
  ...
  return (
    <ScreenContainer scrollable>
      <Stack.Screen options={{ title: 'Withdraw / Spend', headerShown: true }} />
